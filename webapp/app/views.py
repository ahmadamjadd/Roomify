...
from .forms import EmailAuthenticationForm, QuizForm, UpdateForm, UserRegisterForm
from .models import MatchInteraction, RoommateProfile, User
...
def email_user(request, user):
    ...
    try:
        ...
        to_email = user.email
        send_mail(mail_subject, message, os.getenv("EMAIL_ADDRESS"), [to_email])
        return True
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if email_user(request, user):
                ...
                messages.error(
                    request,
                    "Registration failed due to an email error. Please try again.",
                )
                return redirect("register")
    else:
...
def add_phone_number(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "Phone number updated! You are now visible to matches."
            )
            return redirect("dashboard")
        else:
            messages.error(
                request, "Please enter a valid phone number starting with 03."
            )
    return redirect("dashboard")
...
def train_recommender(profiles):
    ...
    print(
        f"ML Recommender System trained and saved on {len(data)} pairs. Profile count: {last_profile_count}"
    )
...
def load_recommender():
    ...
    if trained_model is None and os.path.exists(ML_MODEL_PATH):
        try:
            ...
            print(
                f"ML Model loaded from disk. Last trained with {last_profile_count} profiles."
            )
        except Exception as e:
...
def dashboard_view(request):
    ...
    retrain_needed = (
        ...
        and total_profiles >= last_profile_count + N_NEW_USERS_TO_RETRAIN
    )
    ...
    print(
        f"Total Profiles: {total_profiles}. ML Active: {'YES' if use_ml_recommender else 'NO (Fallback to Heuristic)'}"
    )
    ...
    for other in all_profiles:
        if use_ml_recommender and trained_model:
            ...
            print(
                f"Target: {other.user.username:<10} | Score Source: ML Recommender | Final Score: {final_score:>3}%"
            )
            ...
            print(
                f"Target: {other.user.username:<10} | Score Source: Heuristic | Final Score: {final_score:>3}%"
            )
...
def track_whatsapp_click(request, target_id):
    ...
    try:
        ...
        interaction = MatchInteraction.objects.filter(
            viewer=request.user, target=target_user
        ).first()
...
def metrics_dashboard(request):
    ...
    avg_top_score_data = (
        MatchInteraction.objects.values("viewer")
        .annotate(max_score=Max("match_score"))
        .aggregate(Avg("max_score"))
    )
